...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!linkName || !title) {
      return NextResponse.json({ error: "linkName and title are required" }, { status: 400 });
    }
    ...
    if (existingLink) return NextResponse.json({ error: "Link name already taken" }, { status: 409 });

    // Decrypt mnemonic and derive address on the user's current network
